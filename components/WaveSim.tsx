...
function WaveField({ freq, phase, sep }: { freq: number; phase: number; sep: number }) {
  ...
  const { size } = useThree(); // <- r3f renderer size

  useFrame(() => {
    if (matRef.current) {
      matRef.current.uniforms.u_time.value = (performance.now() - start) / 1000;
    }
  });
  ...
  const uniforms = useMemo(
    ...
    [] // uniforms object is stable; we update values below
  );

  // keep uniforms in sync with props
  useEffect(() => {
    if (matRef.current) matRef.current.uniforms.u_freq.value = freq;
  }, [freq]);
  useEffect(() => {
    if (matRef.current) matRef.current.uniforms.u_phase.value = phase;
  }, [phase]);
  useEffect(() => {
    if (matRef.current) matRef.current.uniforms.u_sep.value = sep;
  }, [sep]);

  // update resolution whenever Canvas size changes (no "any", no unused vars)
  useEffect(() => {
    if (matRef.current) {
